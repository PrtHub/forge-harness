...
export const metadata = {
  title: "Habit blue-prints — Forge",
  description: "Browse the Forge operator habits manuals, configuring friction structures and physical breathing routines.",
};

export default function HabitsIndexPage() {
  const habits = CONTENT_DATABASE.filter((item) => item.category === "habits");
      ...
      <main className="flex-1 px-6 py-16 max-w-[900px] mx-auto w-full">
        ...
        <div className="mb-12">
          ...
          <p className="text-base font-light text-text-muted leading-relaxed max-w-[580px]">
            Practical protocols to construct friction points in your digital space and reset physical tension.
          </p>
        </div>
          ...
          {habits.map((habit) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-3 text-xs text-text-dim mb-3 uppercase font-cinzel tracking-wider">
                <span>{habit.publishDate}</span>
                <span className="w-1 h-1 rounded-full bg-ink-600" />
                <span className="text-ember-500">{habit.readTime}</span>
              </div>
